...
<body>
        ...
        <div class="container-fluid page-body-wrapper">
            ...
            <div class="main-panel">
                ...
                <div class="col-lg-12 grid-margin stretch-card">
                    <div class="card">
                        <div class="card-body">
                            <h4 class="card-title">DATA SOLUSI</h4>
                            ...
                            <div class="table-responsive pt-3">
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td>{{ $solusis->id_penyakit }}</td>
                                            <td>{{ $solusis->solusi }}</td>
                                            <td>
                                                <button type="button" class="btn btn-success m-sm-1 edit-solusi-btn"
                                                    data-toggle="modal" data-target="#modal-edit"
                                                    data-id="{{ $solusis->id }}">Edit</button>
                                                <form action="{{ route('hapus-solusi', $solusis->id_solusi) }}"
                                                    method="POST" id="form-delete-{{ $solusis->id_solusi }}">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
                                                        class="btn btn-danger m-sm-1 delete-solusi-btn">Hapus</button>
                                                </form>
                                            </td>
                                        </tr>
                                        @endforeach
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modal-label">Tambah Solusi</h5>
                ...
                <div class="modal-body">
                    <!-- Form goes here -->
                    <form class="forms-modal" method="POST" action="{{ route('create-solusi')}}">
                        @csrf
    ...
    <div class="modal fade" id="modal-edit" tabindex="-1" role="dialog" aria-labelledby="modal-label"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modal-label">Edit Solusi</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    @foreach($solusi as $solusis)
                    <form class="forms-modal" method="POST" action="{{ route('update-solusi', $solusis->id_solusi) }}">
                        @csrf
                        @method('PUT')
                        <div class="form-group">
                            <label for="id_penyakit">ID Penyakit</label>
                            <select class="form-control" id="id_pennyakit" name="id_penyakit">
                                <option value="">Pilih</option>
                                @foreach($penyakits as $penyakit)
                                <option value="{{ $penyakit->id_penyakit }}"
                                    {{ $penyakit->id_penyakit == $penyakit->id_penyakit ? 'selected' : '' }}>
    ...
    $(document).ready(function() {
        $('.delete-solusi-btn').click(function(event) {
